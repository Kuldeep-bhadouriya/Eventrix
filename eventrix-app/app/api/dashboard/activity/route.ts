...
import { getDashboardActivity } from "@/lib/dashboard/dashboard-queries";
...
export const GET = handleApiError(
  withLogging(async (req, log) => {
    ...
    log.info("Fetching dashboard activity", { userId: session.user.id, limit });

    const activity = await getDashboardActivity(session.user.id, limit ?? 10);
