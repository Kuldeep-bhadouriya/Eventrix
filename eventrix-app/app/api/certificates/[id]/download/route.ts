...
export const GET = handleApiError(
  withLogging(async (req, log) => {
    ...
    // For now we return the resolved URL (if any). Frontend can open it.
    log.info("Requested certificate download", { userId: session.user.id, certificateId });
    return successResponse({ url: details.downloadUrl });
  })
);
